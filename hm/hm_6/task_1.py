...
# ДОБАВИТЬ ИСКЛЮЧЕНИЕ ПО КОЛИЧЕСТВУ ДНЕЙ В МЕСЯЦЕ
def chek_date(date_to_prove):
    temp = date_to_prove.split(".")
    if len(temp) != 3:
        return False
    if 0 < int(temp[0]) <= 31 and 0 < int(temp[1]) <= 12 | 0 < int(temp[2]):
        return True
    else:
        return False
